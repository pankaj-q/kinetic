...
export const ApplicationApprovalModal: React.FC<ApplicationApprovalModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'fields' | 'cover_letter' | 'autofill' | 'history'>('fields');
  ...
  useEffect(() => {
    // Fetch 1-Click Autofill Bookmarklet Script
    fetch(`/api/applications/${application.id}/autofill-script`)
      .then((res) => res.json())
      .then((data) => {
        if (data && data.bookmarklet) {
          setBookmarklet(data.bookmarklet);
        }
      })
      .catch((err) => console.warn('Could not fetch autofill script:', err));
  }, [application.id]);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 border-b border-[#1D1D24] bg-[#0E0E13]">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>⚡ 1-Click Autofill Script</span>
            </button>
            <button
          ...
          <div className="flex items-center gap-2">
            {activeTab === 'autofill' && (
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#FF5A36]/15 hover:bg-[#FF5A36]/25 text-[#FF5A36] text-[11px] font-bold transition-all border border-[#FF5A36]/30"
              >
                {copiedBookmarklet ? <Check className="w-3 h-3 text-[#00FF88]" /> : <Copy className="w-3 h-3" />}
                <span>{copiedBookmarklet ? 'Copied Autofill JS!' : 'Copy 1-Click Script'}</span>
              </button>
            )}
            {activeTab === 'fields' && (
              <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#111116]">
          {/* Submission Receipt Banner (If already applied) */}
          {application.status === 'APPLIED' && (
            <div className="p-4 rounded-xl bg-[#00FF88]/5 border border-[#00FF88]/30 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-[#00FF88]/15 border border-[#00FF88]/40 flex items-center justify-center text-[#00FF88]">
                    <CheckCircle2 className="w-4 h-4" />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-white font-display">Verified Live Submission Receipt</h4>
                    <p className="text-[11px] text-[#00FF88] font-mono">
                      Channel: {application.submissionChannel || 'Direct Multi-Channel Pipeline'}
                    </p>
                  </div>
                </div>
                {application.submissionReceipt?.receiptId && (
                  <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-[#16161E] text-white border border-[#1D1D24]">
                    ID: {application.submissionReceipt.receiptId}
                  </span>
                )}
              </div>
              {application.submissionReceipt?.details && (
                <p className="text-xs text-[#CCCCCC] bg-[#0E0E14] p-2.5 rounded-lg border border-[#1D1D24] font-mono">
                  {application.submissionReceipt.details}
                </p>
              )}
            </div>
          ...
          {activeTab === 'autofill' && (
            <div className="space-y-5">
              <div className="p-5 rounded-2xl bg-[#14141E] border border-[#FF5A36]/30 space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-[#FF5A36]/15 text-[#FF5A36] border border-[#FF5A36]/30">
                        UNIVERSAL DOM INJECTOR
                      </span>
                      <h3 className="text-base font-bold text-white font-display">
                        1-Click Smart Autofill Bookmarklet
                      </h3>
                    </div>
                    <p className="text-xs text-[#8E8E9B] mt-1.5 leading-relaxed">
                      Instantly populate all 20+ form inputs on Greenhouse, Lever, Workday, Ashby, LinkedIn, or any company career portal in under 0.5s.
                    </p>
                  </div>
                </div>

                {/* Drag and Copy Action */}
                <div className="flex flex-wrap items-center gap-3 pt-2">
                  <a
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#FF5A36] hover:bg-[#FF451A] text-white text-xs font-bold shadow-lg shadow-[#FF5A36]/20 transition-all cursor-grab active:cursor-grabbing"
                    title="Drag this button directly to your browser's Bookmarks Bar"
                  >
                    <Zap className="w-4 h-4 fill-white" />
                    <span>⚡ Drag to Bookmarks Bar: Autofill ({application.company})</span>
                  </a>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[#1E1E2A] hover:bg-[#282838] text-white text-xs font-bold border border-[#2D2D3E] transition-all"
                  >
                    {copiedBookmarklet ? <Check className="w-4 h-4 text-[#00FF88]" /> : <Copy className="w-4 h-4 text-[#FF5A36]" />}
                    <span>{copiedBookmarklet ? 'Script Copied to Clipboard!' : 'Copy JavaScript Code'}</span>
                  </button>
                  ...
                  {application.applicationUrl && application.applicationUrl !== '#' && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[#111116] hover:bg-[#181822] text-[#FF5A36] text-xs font-bold border border-[#FF5A36]/30 transition-all"
                    >
                      <span>1. Open Application URL</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-3 border-t border-[#1D1D24]">
                  <div className="p-3 rounded-xl bg-[#0B0B0E] border border-[#1D1D24] space-y-1">
                    <span className="text-[10px] font-mono font-bold text-[#FF5A36]">STEP 1</span>
                    <h5 className="text-xs font-bold text-white">Save Bookmarklet</h5>
                    <p className="text-[11px] text-[#8E8E9B]">
                      Drag the orange button above to your browser bookmarks bar (or copy the JS script).
                    </p>
                  </div>
                  <div className="p-3 rounded-xl bg-[#0B0B0E] border border-[#1D1D24] space-y-1">
                    <span className="text-[10px] font-mono font-bold text-[#FF5A36]">STEP 2</span>
                    <h5 className="text-xs font-bold text-white">Open Job Portal</h5>
                    <p className="text-[11px] text-[#8E8E9B]">
                      Click "Open Application URL" to go to the official job listing at {application.company}.
                    </p>
                  </div>
                  <div className="p-3 rounded-xl bg-[#0B0B0E] border border-[#1D1D24] space-y-1">
                    <span className="text-[10px] font-mono font-bold text-[#00FF88]">STEP 3</span>
                    <h5 className="text-xs font-bold text-white">Click & Auto-Fill</h5>
                    <p className="text-[11px] text-[#8E8E9B]">
                      Click the bookmark in your bar. All fields, custom questions, and contact info fill instantly.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono text-[#8E8E9B]">Generated DOM Payload Preview</span>
                  <span className="text-[11px] text-[#00FF88] font-mono">100% Client-Side Safe</span>
                </div>
                <div className="bg-[#070709] border border-[#1D1D24] rounded-xl p-3.5 max-h-36 overflow-y-auto font-mono text-[11px] text-[#8E8E9B] break-all leading-relaxed">
                  {bookmarklet || '// Loading bookmarklet script...'}
                </div>
              </div>
            </div>
          )}
