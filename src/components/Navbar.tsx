    ...
    <header className="sticky top-0 z-40 w-full border-b border-[#1D1D24] bg-[#070709]/95 backdrop-blur-md transition-all font-['Geist',sans-serif]">
      ...
      <div className="w-full border-b border-[#1D1D24] bg-[#0A0A0E] py-1.5 overflow-hidden relative group/marquee [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)]">
        <div className="animate-marquee-slow flex items-center justify-center gap-6 text-xs select-none font-mono">
          ...
          <div className="flex items-center gap-2 px-3 py-0.5 rounded-full bg-[#111116] border border-[#1D1D24] text-[#8E8E9B] text-[11px] whitespace-nowrap shrink-0">
            ...
            <span>TELEGRAM & GMAIL: <strong className="text-[#FFFFFF] font-bold">LIVE SYNCED</strong></span>
          </div>

          <div className="flex items-center gap-2 px-3 py-0.5 rounded-full bg-[#111116] border border-[#1D1D24] text-[#8E8E9B] text-[11px] whitespace-nowrap shrink-0">
            <Bot className="w-3 h-3 text-[#FF5A36]" />
            <span>AI AGENT: <strong className="text-[#FFFFFF] font-bold">GEMINI 2.0 FLASH</strong></span>
          </div>

          <div className="flex items-center gap-2 px-3 py-0.5 rounded-full bg-[#111116] border border-[#1D1D24] text-[#8E8E9B] text-[11px] whitespace-nowrap shrink-0">
            <Flame className="w-3 h-3 text-[#FF5A36]" />
            <span>MATCH ACCURACY: <strong className="text-[#00FF88] font-bold">94%+ FIT</strong></span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            >
              ...
              <div className="flex items-baseline gap-2">
                <span className="font-display text-xl font-bold tracking-wider text-[#FFFFFF] uppercase">
                  Kinetic
                </span>
          ...
          {activeTab === 'landing' ? (
            <nav className="hidden lg:flex items-center gap-6 text-[13px] font-medium text-[#8E8E9B]">
              <a href="#product" className="hover:text-[#FFFFFF] transition-colors">Product</a>
              ...
              <a href="#developers" className="hover:text-[#FFFFFF] transition-colors">Developers</a>
              <button
              ...
              <button
                ...
                className="hover:text-[#FFFFFF] transition-colors cursor-pointer flex items-center gap-1.5"
              >
                <span>Agent Console</span>
                {isAgentRunning && <span className="w-1.5 h-1.5 rounded-full bg-[#00FF88] animate-pulse" />}
              </button>
            </nav>
          ) : (
            <nav className="hidden lg:flex items-center gap-1 bg-[#111116] p-1 rounded-xl border border-[#1D1D24]">
                  ...
                  >
                    {isActive && (
                      <motion.div
                        ...
                        className="absolute inset-0 bg-[#1D1D24] border border-[#2D2D38] rounded-lg -z-10 shadow-xs"
                    ...
                    {item.badge && (
                      <span className="px-1.5 py-0.2 rounded-full text-[9px] font-mono font-bold bg-[#FF5A36]/20 text-[#FF5A36] animate-pulse">
                        {item.badge}
                      </span>
                    )}
                  </button>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#1D1D24] bg-[#111116] text-xs font-mono text-[#8E8E9B] hover:text-[#FFFFFF] hover:border-[#2D2D38] transition-all"
            >
              <span>GitHub</span>
              <span className="text-[#FFFFFF] font-semibold">★ 4.8k</span>
            </a>
            ...
            <div className="relative">
              <button
                ...
                className="p-2 rounded-lg border border-[#1D1D24] bg-[#111116] text-[#8E8E9B] hover:text-[#FFFFFF] hover:border-[#2D2D38] transition-all relative cursor-pointer"
              ...
              <AnimatePresence>
                {showNotifPopover && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-80 sm:w-96 rounded-xl bg-[#111116] py-2 z-50 border border-[#1D1D24] shadow-2xl backdrop-blur-xl"
                  >
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#1D1D24]">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold font-mono tracking-tight text-[#FFFFFF] uppercase">Live Telemetry Stream</span>
                        {unreadCount > 0 && (
                          <span className="px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-[#FF5A36]/20 text-[#FF5A36]">
                            {unreadCount} new
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        {unreadCount > 0 && (
                          <button
                            ...
                            className="text-[10px] text-[#8E8E9B] hover:text-[#FFFFFF] transition-colors cursor-pointer"
                          >
                            Mark all read
                          </button>
                        )}
                    ...
                    <div className="max-h-80 overflow-y-auto divide-y divide-[#1D1D24]">
                      {notifications.length === 0 ? (
                        <div className="p-6 text-center text-xs text-[#8E8E9B]">
                        ...
                        notifications.map((n) => (
                          <div
                            ...
                            className={`p-3.5 text-xs cursor-pointer hover:bg-[#16161E] transition-colors flex items-start gap-2.5 ${
                              !n.read ? 'bg-[#FF5A36]/10' : ''
                            }`}
                          >
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-semibold text-[#FFFFFF]">{n.title}</span>
                              ...
                              <p className="text-xs text-[#8E8E9B] mt-1 line-clamp-2 leading-relaxed">
                                {n.body}
                              </p>
        ...
        <div className="lg:hidden flex items-center gap-1 overflow-x-auto py-2.5 border-t border-[#1D1D24] -mx-4 px-4 scrollbar-none">
          {workbenchNavItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                  isActive
                    ? 'bg-[#1D1D24] text-[#FFFFFF] border border-[#2D2D38]'
                    : 'text-[#8E8E9B] hover:text-[#FFFFFF] bg-[#111116] border border-[#1D1D24]'
                }`}
              >
